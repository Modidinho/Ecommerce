...
if($pesapalNotification=="CHANGE" && $pesapalTrackingId!='')
{
   ...
   $sql = mysqli_query($dbc,"UPDATE payments SET
   ...
   $payment = mysqli_fetch_array(mysqli_query($dbc,"SELECT id FROM payments WHERE pesapal_transaction_tracking_id='$pesapalTrackingId' && reference='$pesapal_merchant_reference' "));

   $payment_id = $payment['id'];
   
   $update_orders = mysqli_query($dbc,"UPDATE orders SET status='processing' WHERE payment_id='".$payment_id."' ");

   if($sql && $update_orders)
   {
      ...
      header('Location:'.app_url.'?payment_success='.$success.'');
      exit;
   }
   else 
   {
